...
import csv  #module used to get data from csv file ...
...
def readMarkSheet(file_name):
    ...
    csvreader = csv.reader(input_file_obj)
    next(csvreader)
    for row in csvreader:
        name_marks_mapping [row[0]]={'marks':[row[i] for i in range(1,6)]}
...
def generateGradeCard(mapping_dict):
    ...
    for names in name_marks_mapping:
        sum = 0
        avg = 0
        for ele in name_marks_mapping[names]['marks']:
            avg += int(ele)
        percentage = avg//5
        if(sum >= 90):
            grd = 'O'
        ...
        grade_card[names]={'subject_wise_marks': name_marks_mapping[names]['marks'],'grade_received':grd}
